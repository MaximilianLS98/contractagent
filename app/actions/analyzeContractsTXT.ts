...
const analysisResultsCollectionId = process.env
	.NEXT_PUBLIC_APPWRITE_COLLECTION_ID_HISTORY as string;
...
export async function analyzeTXTContract(formData: FormData) {
	...
	if (userQuota.document_quota_left === 0) {
		return {
			data: null,
			error: 'Du er tom for tokens, for å analysere flere dokumenter må du kjøpe flere.',
		};
	}
	...
	try {
		...
		const analysisResult = responseObj[0];

		if (!analysisResult) {
			console.error('No assistant message found in the response.');
			return { data: null, error: 'Analysis completed but no result found.' };
		}

		try {
			if (!analysisResultsCollectionId) {
				throw new Error(
					'Analysis results collection ID is not configured in environment variables (NEXT_PUBLIC_APPWRITE_COLLECTION_ID_HISTORY).',
				);
			}
			const savedResult = await databases.createDocument(
				dbId,
				analysisResultsCollectionId,
				ID.unique(),
				{
					user_id: userId,
					content: fileAsText,
			...
			console.log(
				`Analysis result saved (ID: ${savedResult.$id}) for user ${userId} and file ${file.name}`,
			);
		} catch (dbError) {
			console.error('Failed to save analysis result to Appwrite:', dbError);
		}

		const updatedUserQueryObj = await databases.updateDocument(
			dbId,
			collectionId,
			userQuota.$id,
			{
		...
		// Consider revalidating a tag for analysis history if you implement fetching it
		// revalidateTag('analysisHistory');

		// Return the actual analysis result object, not just the first content block
